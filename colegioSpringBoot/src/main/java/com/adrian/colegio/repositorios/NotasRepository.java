...
public interface NotasRepository extends CrudRepository<NotasEntity, Integer> {
	@Query("""
			...
			JOIN n.alumno al
			WHERE
			(:idAlumno IS NULL OR al.id = :idAlumno)
	...
	ArrayList<NotasDTO> buscarNotasPorFiltros(@Param("idAlumno") Integer idAlumno,
			@Param("nombreAlumno") String nombreAlumno, @Param("nombreAsignatura") String nombreAsignatura,
			@Param("nota") Float nota, @Param("fecha") String fecha, @Param("activo") int activo);
}